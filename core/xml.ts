...
export function workspaceToDom(
  ...
  // AnyDuringMigration because:  Property 'get' does not exist on type '(name:
  // string) => void'.
  const variablesElement = variablesToDom((goog.module as AnyDuringMigration)
    .get('Blockly.Variables')
    .allUsedVarModels(workspace));
    ...
    for (let i = 0, xmlChild; xmlChild = xml.childNodes[i]; i++) {
      ...
      if (name === 'block' ||
        ...
        if (workspace.rendered) {
          ...
          // '(name: string) => void'.
          const { WorkspaceCommentSvg } = (goog.module as AnyDuringMigration)
            .get('Blockly.WorkspaceCommentSvg');
          ...
          // AnyDuringMigration because:  Property 'get' does not exist on type
          // '(name: string) => void'.
          const { WorkspaceComment } = (goog.module as AnyDuringMigration)
            .get('Blockly.WorkspaceComment');
          if (!WorkspaceComment) {
            console.warn(
...
export function appendDomToWorkspace(
  xml: Element, workspace: Workspace): string[] {
...
export function domToBlock(xmlBlock: Element, workspace: Workspace): Block {
  ...
  if (eventUtils.isEnabled()) {
    ...
    // '(name: string) => void'.
    const newVariables =
      (goog.module as AnyDuringMigration)
        .get('Blockly.Variables')
  ...
  for (let i = 0; i < xmlBlock.children.length; i++) {
    ...
    switch (xmlChild.nodeName.toLowerCase()) {
      ...
      case 'comment':
        // AnyDuringMigration because:  Property 'get' does not exist on type
        // '(name: string) => void'.
        if (!(goog.module as AnyDuringMigration).get('Blockly.Comment')) {
          console.warn(
 ...
 */
function findChildBlocks(xmlNode: Element): { childBlockElement: Element | null, childShadowElement: Element | null } {
