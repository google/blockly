...
JavaScript['procedures_defreturn'] = function(block) {
  ...
  if (JavaScript.STATEMENT_SUFFIX) {
    xfix1 += JavaScript.injectId(JavaScript.STATEMENT_SUFFIX,
        block);
  }
  if (xfix1) {
  ...
  if (JavaScript.INFINITE_LOOP_TRAP) {
    loopTrap = JavaScript.prefixLines(
        JavaScript.injectId(JavaScript.INFINITE_LOOP_TRAP,
        block), JavaScript.INDENT);
  }
  const branch = JavaScript.statementToCode(block, 'STACK');
  let returnValue = JavaScript.valueToCode(block, 'RETURN',
...
JavaScript['procedures_defnoreturn'] =
    JavaScript['procedures_defreturn'];
...
JavaScript['procedures_callreturn'] = function(block) {
  ...
  const variables = block.getVars();
  for (let i = 0; i < variables.length; i++) {
    args[i] = JavaScript.valueToCode(block, 'ARG' + i,
...
JavaScript['procedures_ifreturn'] = function(block) {
  ...
  const condition = JavaScript.valueToCode(block, 'CONDITION',
