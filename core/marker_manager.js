 ...
 * @param {!Blockly.WorkspaceSvg} workspace The workspace for the marker
 *     manager.
 * @constructor
 * @package
 */
...
Blockly.MarkerManager.prototype.unregisterMarker = function(id) {
  ...
  if (marker) {
    ...
    throw Error(
        'Marker with id ' + id + ' does not exist. Can only unregister' +
        'markers that exist.');
  }
};
...
Blockly.MarkerManager.prototype.setCursor = function(cursor) {
  ...
  this.cursor_ = cursor;
  if (this.cursor_) {
    var drawer = this.workspace_.getRenderer().makeMarkerDrawer(
