...
Lua['colour_rgb'] = function(block) {
  ...
  const functionName = Lua.provideFunction_(
      ...
      ['function ' + Lua.FUNCTION_NAME_PLACEHOLDER_ + '(r, g, b)',
       ...
       'end']);
  const r = Lua.valueToCode(block, 'RED',
      Lua.ORDER_NONE) || 0;
  const g = Lua.valueToCode(block, 'GREEN',
      Lua.ORDER_NONE) || 0;
  const b = Lua.valueToCode(block, 'BLUE',
      Lua.ORDER_NONE) || 0;
...
Lua['colour_blend'] = function(block) {
  ...
  const functionName = Lua.provideFunction_(
      ...
      ['function ' + Lua.FUNCTION_NAME_PLACEHOLDER_ +
       ...
       '  return string.format("#%02x%02x%02x", r, g, b)',
       'end']);
  const colour1 = Lua.valueToCode(block, 'COLOUR1',
      Lua.ORDER_NONE) || '\'#000000\'';
  const colour2 = Lua.valueToCode(block, 'COLOUR2',
      Lua.ORDER_NONE) || '\'#000000\'';
  const ratio = Lua.valueToCode(block, 'RATIO',
      Lua.ORDER_NONE) || 0;
