...
/**
 * @fileoverview Generating SQL for list blocks.
 * @author daarond@gmail.com (Daaron Dwyer)
 */
'use strict';

goog.provide('Blockly.SQL.lists');

goog.require('Blockly.SQL');


Blockly.SQL['lists_create_empty'] = function(block) {
  // Create an empty list.
  return ['array()', Blockly.SQL.ORDER_ATOMIC];
};

Blockly.SQL['lists_create_with'] = function(block) {
...
Blockly.SQL['lists_repeat'] = function(block) {
  ...
  var functionName = Blockly.SQL.provideFunction_(
      ...
      [ 'function ' + Blockly.SQL.FUNCTION_NAME_PLACEHOLDER_ +
        ...
        '  return $array;',
        '}']);
  var argument0 = Blockly.SQL.valueToCode(block, 'ITEM',
      Blockly.SQL.ORDER_COMMA) || 'null';
  var argument1 = Blockly.SQL.valueToCode(block, 'NUM',
      Blockly.SQL.ORDER_COMMA) || '0';
  var code = functionName + '(' + argument0 + ', ' + argument1 + ')';
  return [code, Blockly.SQL.ORDER_FUNCTION_CALL];
};

Blockly.SQL['lists_length'] = function(block) {
...
Blockly.SQL['lists_isEmpty'] = function(block) {
...
Blockly.SQL['lists_indexOf'] = function(block) {
  ...
  var argument1 = Blockly.SQL.valueToCode(block, 'VALUE',
      Blockly.SQL.ORDER_MEMBER) || '[]';
  var functionName;
  if (block.getFieldValue('END') == 'FIRST'){
    // indexOf
    functionName = Blockly.SQL.provideFunction_(
        'indexOf',
        [ 'function ' + Blockly.SQL.FUNCTION_NAME_PLACEHOLDER_ +
        '($haystack, $needle) {',
...
Blockly.SQL['lists_getIndex'] = function(block) {
  // Get element at index.
  var mode = block.getFieldValue('MODE') || 'GET';
  var where = block.getFieldValue('WHERE') || 'FROM_START';
  var at = Blockly.SQL.valueToCode(block, 'AT',
      Blockly.SQL.ORDER_UNARY_NEGATION) || '1';
  var list = Blockly.SQL.valueToCode(block, 'VALUE',
      Blockly.SQL.ORDER_FUNCTION_CALL) || 'array()';
...
Blockly.SQL['lists_setIndex'] = function(block) {
  // Set element at index.
  // Note: Until February 2013 this block did not have MODE or WHERE inputs.
  var list = Blockly.SQL.valueToCode(block, 'LIST',
      Blockly.SQL.ORDER_MEMBER) || 'array()';
  var mode = block.getFieldValue('MODE') || 'GET';
  var where = block.getFieldValue('WHERE') || 'FROM_START';
  var at = Blockly.SQL.valueToCode(block, 'AT',
      Blockly.SQL.ORDER_NONE) || '1';
  var value = Blockly.SQL.valueToCode(block, 'TO',
      Blockly.SQL.ORDER_ASSIGNMENT) || 'null';
  ...
  function cacheList() {
    ...
    var listVar = Blockly.SQL.variableDB_.getDistinctName(
  ...
  if (where == 'FIRST') {
    ...
    var code = cacheList();
    var xVar = Blockly.SQL.variableDB_.getDistinctName(
...
Blockly.SQL['lists_getSublist'] = function(block) {
  // Get sublist.
  var list = Blockly.SQL.valueToCode(block, 'LIST',
      Blockly.SQL.ORDER_MEMBER) || 'array()';
...
Blockly.SQL['lists_split'] = function(block) {
  // Block for splitting text into a list, or joining a list into text.
  var value_input = Blockly.SQL.valueToCode(block, 'INPUT',
      Blockly.SQL.ORDER_MEMBER);
  var value_delim = Blockly.SQL.valueToCode(block, 'DELIM',
      Blockly.SQL.ORDER_NONE) || '\'\'';
