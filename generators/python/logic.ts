...
export function logic_compare(block: Block, generator: PythonGenerator): [string, Order] {
  // Comparison operator.
  const OPERATORS =
...
export function logic_operation(block: Block, generator: PythonGenerator): [string, Order] {
  // Operations 'and', 'or'.
...
export function logic_negate(block: Block, generator: PythonGenerator): [string, Order] {
  // Negation.
  const argument0 =
...
export function logic_boolean(block: Block, generator: PythonGenerator): [string, Order] {
  // Boolean values true and false.
  const code = (block.getFieldValue('BOOL') === 'TRUE') ? 'True' : 'False';
  return [code, Order.ATOMIC];
};

export function logic_null(block: Block, generator: PythonGenerator): [string, Order] {
  // Null data type.
  return ['None', Order.ATOMIC];
};

export function logic_ternary(block: Block, generator: PythonGenerator): [string, Order] {
  // Ternary operator.
  const value_if =
