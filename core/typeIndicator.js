...
(function() {
  ...
  * blocks have moved then we dont need to worry about moving the indicators
 ...
 Blockly.BlockSvg.prototype.onMouseMove_ = function(e) {
  var this_ = this; // we need to dave the this context so the command beeing created can access it
  Blockly.doCommand(function() {
   if (Blockly.dragMode_ == 2 && !draggingWorkspace) { // Only on first drag move
    ...
    if (this_.outputConnection) {
     ...
     typ = "input";
    } else if (this_.previousConnection) {
