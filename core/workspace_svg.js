...
class WorkspaceSvg extends Workspace {
  ...
  constructor(options, opt_blockDragSurface, opt_wsDragSurface) {
    ...
    this.massOperationsHandler_ = null;
  }
  ...
  resize(keepCenter) {
    ...
    if (this.scrollbar) {
      this.scrollbar.resize(keepCenter);
    }
   ...
   * @private
   */
  pasteBlock_(xmlBlock, jsonBlock, {dontSelectNewBLock} = {}) {
    eventUtils.disable();
  ...
  getTopBlocks(ordered, inActiveModule) {
    return super.getTopBlocks(ordered, inActiveModule);
  }
