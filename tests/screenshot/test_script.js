...
suite('Rendering', function() {
  test('math_addition', function(done) {
    function doneReading() {
      ...
      diff.pack().pipe(fs.createWriteStream(diff_dir + 'my_screenshot.png'));
      chai.assert.equal(mismatch_num, 0);
      done();
    }

    var img1 = fs.createReadStream(old_dir + 'my_screenshot.png')
        .pipe(new PNG()).on('parsed', doneReading);
    var img2 = fs.createReadStream(new_dir + 'my_screenshot.png')
        .pipe(new PNG()).on('parsed', doneReading);
    var filesRead = 0;
  });
});
