...
class BlockSvg extends Block {
  ...
  getRelativeToSurfaceXY() {
    ...
    if (element) {
      ...
      if (this.useDragSurface_ && this.workspace.getBlockDragSurface().getCurrentBlock() ===
                element) {
   ...
   * @package
   */
  prepareForMoveToDragSurface(positionOnDragSurface) {
   ...
   * @package
   */
  moveToDragSurface(positionOnDragSurface) {
   ...
   * @package
   */
  toCopyData(addNextBlocks = false) {
    if (this.isInsertionMarker_) {
      return null;
    }
    ...
    return {
      saveInfo: /** @type {!blocks.State} */ (
          blocks.save(this, {addCoordinates: true, addNextBlocks})),
