...
JavaScript['lists_repeat'] = function(block) {
  ...
  const functionName = JavaScript.provideFunction_(
      ...
      ['function ' + JavaScript.FUNCTION_NAME_PLACEHOLDER_ +
       ...
       '  return array;',
       '}']);
  const element = JavaScript.valueToCode(block, 'ITEM',
      JavaScript.ORDER_NONE) || 'null';
  const repeatCount = JavaScript.valueToCode(block, 'NUM',
...
JavaScript['lists_length'] = function(block) {
  ...
  const list = JavaScript.valueToCode(block, 'VALUE',
...
JavaScript['lists_indexOf'] = function(block) {
  ...
  const operator = block.getFieldValue('END') === 'FIRST' ?
      'indexOf' : 'lastIndexOf';
  const item = JavaScript.valueToCode(block, 'FIND',
...
JavaScript['lists_getIndex'] = function(block) {
  ...
  const listOrder = (where === 'RANDOM') ? JavaScript.ORDER_NONE :
      JavaScript.ORDER_MEMBER;
...
JavaScript['lists_setIndex'] = function(block) {
  ...
  let list = JavaScript.valueToCode(block, 'LIST',
      JavaScript.ORDER_MEMBER) || '[]';
  const mode = block.getFieldValue('MODE') || 'GET';
  const where = block.getFieldValue('WHERE') || 'FROM_START';
  const value = JavaScript.valueToCode(block, 'TO',
      JavaScript.ORDER_ASSIGNMENT) || 'null';
  ...
  switch (where) {
    ...
    case ('FROM_END'): {
      const at = JavaScript.getAdjusted(block, 'AT', 1, false,
...
JavaScript['lists_getSublist'] = function(block) {
  // Get sublist.
  const list = JavaScript.valueToCode(block, 'LIST',
      JavaScript.ORDER_MEMBER) || '[]';
...
JavaScript['lists_sort'] = function(block) {
  ...
  const getCompareFunctionName = JavaScript.provideFunction_(
      'listsGetSortCompare',
      ['function ' + JavaScript.FUNCTION_NAME_PLACEHOLDER_ +
          '(type, direction) {',
       '  var compareFuncs = {',
...
JavaScript['lists_split'] = function(block) {
  ...
  let input = JavaScript.valueToCode(block, 'INPUT',
      JavaScript.ORDER_MEMBER);
  const delimiter = JavaScript.valueToCode(block, 'DELIM',
      JavaScript.ORDER_NONE) || '\'\'';
