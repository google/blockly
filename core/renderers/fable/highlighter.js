...
goog.provide('Blockly.fable.Highlighter');
 ...
 * @constructor
 */
Blockly.fable.Highlighter = function (info) {
  this.info_ = info;
  this.steps_ = '';
  this.inlineSteps_ = '';
  ...
  var renderer = /** @type {!Blockly.fable.Renderer} */ (info.getRenderer());
 ...
 * @package
 */
Blockly.fable.Highlighter.prototype.getPath = function () {
  return this.steps_ + '\n' + this.inlineSteps_;
};

Blockly.fable.Highlighter.prototype.drawTopCorner = function (row) {
...
Blockly.fable.Highlighter.prototype.drawJaggedEdge_ = function (row) {
  if (this.info_.RTL) {
...
Blockly.fable.Highlighter.prototype.drawValueInput = function (row, rowHeight) {
  var input = row.getLastInput();
  var belowTabHeight = rowHeight - input.connectionHeight;

  if (this.RTL_) {
    this.steps_ +=
        Blockly.utils.svgPaths.moveTo(
          input.xPos + input.width - this.highlightOffset_, row.yPos + (belowTabHeight / 2)) +
        this.puzzleTabPaths_.pathDown(this.RTL_) +
        Blockly.utils.svgPaths.lineOnAxis('v', (belowTabHeight / 2));
  } else {
    this.steps_ +=
        Blockly.utils.svgPaths.moveTo(input.xPos + input.width, row.yPos + (belowTabHeight / 2)) +
        this.puzzleTabPaths_.pathDown(this.RTL_);
  }
};

Blockly.fable.Highlighter.prototype.drawStatementInput = function (row) {
  var input = row.getLastInput();
  if (this.RTL_) {
...
Blockly.fable.Highlighter.prototype.drawRightSideRow = function (row) {
...
Blockly.fable.Highlighter.prototype.drawBottomRow = function (row) {
...
Blockly.fable.Highlighter.prototype.drawLeft = function () {
  var outputConnection = this.info_.outputConnection;
  if (outputConnection) {
    var tabBottom = this.info_.tabBottom_;
...
Blockly.fable.Highlighter.prototype.drawInlineInput = function (input) {
  ...
  if (this.RTL_) {
    ...
    this.inlineSteps_ +=
        ...
        Blockly.utils.svgPaths.lineOnAxis('h', -bottomHighlightWidth) +
        // Go to top of tab.
        Blockly.utils.svgPaths.moveTo(connectionRight, yPos + verticalOffset) +
