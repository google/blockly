...
Blockly.MostUsed.init = function () {
  const blockFrequency = localStorage.getItem('blockFrequency');
  const blockDefinition = localStorage.getItem('blockDefinition');
...
Blockly.MostUsed.getMostFrequentBlocks = function (howMany) {
  const items = Object.keys(Blockly.MostUsed.blockCounterDict).map(function (key) {
...
Blockly.MostUsed.flyoutCategory = function () {
  ...
  const orderedByFreq = Blockly.MostUsed.getMostFrequentBlocks(Blockly.MostUsed.MAX_BLOCKS_TO_DISPLAY);
  ...
  for (let i = 0; i < Blockly.MostUsed.MAX_BLOCKS_TO_DISPLAY; i++) {
    ...
    const blockDefinition = Blockly.MostUsed.blockDefinitionDict[orderedByFreq[i][0]];
    const definitionXML = new DOMParser().parseFromString(blockDefinition, 'text/xml');
 ...
 */
Blockly.MostUsed.incrementCounter = function (clickedBlock, targetWorkspace) {
  ...
  if (targetWorkspace !== Blockly.mainWorkspace) {
    return;
  }
  ...
  if (blockType in Blockly.MostUsed.blockCounterDict) {
    ...
    const blockDefinition = Blockly.Xml.blockToDom(clickedBlock, true);
