...
Dart['text_multiline'] = function(block) {
  ...
  const order = code.indexOf('+') !== -1 ? Dart.ORDER_ADDITIVE :
      Dart.ORDER_ATOMIC;
  return [code, order];
};
...
Dart['text_join'] = function(block) {
  ...
  switch (block.itemCount_) {
    ...
    case 1: {
      const element = Dart.valueToCode(block, 'ADD0',
...
Dart['text_append'] = function(block) {
  ...
  const varName = Dart.nameDB_.getName(block.getFieldValue('VAR'),
      NameType.VARIABLE);
  const value = Dart.valueToCode(block, 'TEXT',
      Dart.ORDER_NONE) || '\'\'';
...
Dart['text_length'] = function(block) {
  ...
  const text = Dart.valueToCode(block, 'VALUE',
      Dart.ORDER_UNARY_POSTFIX) || '\'\'';
...
Dart['text_indexOf'] = function(block) {
  ...
  const operator = block.getFieldValue('END') === 'FIRST' ?
      'indexOf' : 'lastIndexOf';
  const substring = Dart.valueToCode(block, 'FIND',
      Dart.ORDER_NONE) || '\'\'';
  const text = Dart.valueToCode(block, 'VALUE',
      Dart.ORDER_UNARY_POSTFIX) || '\'\'';
...
Dart['text_charAt'] = function(block) {
  ...
  const textOrder = (where === 'FIRST' || where === 'FROM_START') ?
      Dart.ORDER_UNARY_POSTFIX : Dart.ORDER_NONE;
...
Dart['text_getSubstring'] = function(block) {
  ...
  const textOrder = requiresLengthCall ? Dart.ORDER_UNARY_POSTFIX :
      Dart.ORDER_NONE;
...
Dart['text_changeCase'] = function(block) {
  ...
  const textOrder = operator ? Dart.ORDER_UNARY_POSTFIX :
      Dart.ORDER_NONE;
...
Dart['text_trim'] = function(block) {
  ...
  const text = Dart.valueToCode(block, 'TEXT',
      Dart.ORDER_UNARY_POSTFIX) || '\'\'';
...
Dart['text_print'] = function(block) {
  // Print statement.
  const msg = Dart.valueToCode(block, 'TEXT',
      Dart.ORDER_NONE) || '\'\'';
  return 'print(' + msg + ');\n';
};
...
Dart['text_prompt_ext'] = function(block) {
  // Prompt function.
  Dart.definitions_['import_dart_html'] =
  ...
  if (block.getField('TEXT')) {
    ...
    // External message.
    msg = Dart.valueToCode(block, 'TEXT',
        Dart.ORDER_NONE) || '\'\'';
  }
...
Dart['text_count'] = function(block) {
  const text = Dart.valueToCode(block, 'TEXT',
      Dart.ORDER_NONE) || '\'\'';
  const sub = Dart.valueToCode(block, 'SUB',
      Dart.ORDER_NONE) || '\'\'';
...
Dart['text_replace'] = function(block) {
  const text = Dart.valueToCode(block, 'TEXT',
      Dart.ORDER_UNARY_POSTFIX) || '\'\'';
  const from = Dart.valueToCode(block, 'FROM',
      Dart.ORDER_NONE) || '\'\'';
  const to = Dart.valueToCode(block, 'TO',
      Dart.ORDER_NONE) || '\'\'';
