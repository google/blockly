...
Blockly.Flyout.prototype.createDom = function(tagName) {
  ...
  */
  ...
  this.svgBackground_ = Blockly.utils.dom.createSvgElement(
      'path', {'class': 'blocklyFlyoutBackground'}, this.svgGroup_);
...
Blockly.Flyout.prototype.init = function(targetWorkspace) {
  ...
  this.scrollbar = new Blockly.Scrollbar(
      this.workspace_, this.horizontalLayout, false, 'blocklyFlyoutScrollbar');
 ...
 *    array of Nodes, a NodeList or a string with the name of the dynamic
 * category. Variables and procedures have a custom set of blocks.
 */
Blockly.Flyout.prototype.show = function(flyoutDef) {
  ...
  if (typeof flyoutDef == 'string') {
    var fnToApply =
        this.workspace_.targetWorkspace.getToolboxCategoryCallback(flyoutDef);
...
Blockly.Flyout.prototype.createFlyoutInfo_ = function(parsedContent) {
  ...
  for (var i = 0, contentInfo; (contentInfo = parsedContent[i]); i++) {
    switch (contentInfo['kind'].toUpperCase()) {
      case 'BLOCK':
        var blockInfo =
            /** @type {Blockly.utils.toolbox.Block} */ (contentInfo);
      ...
      case 'SEP':
        var sepInfo =
            /** @type {Blockly.utils.toolbox.Separator} */ (contentInfo);
        this.addSeparatorGap_(sepInfo, gaps, defaultGap);
        break;
      case 'LABEL':
        var labelInfo =
            /** @type {Blockly.utils.toolbox.Label} */ (contentInfo);
      ...
      case 'BUTTON':
        var buttonInfo =
            /** @type {Blockly.utils.toolbox.Button} */ (contentInfo);
