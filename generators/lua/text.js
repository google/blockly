...
Lua['text_multiline'] = function(block) {
  ...
  const order = code.indexOf('..') !== -1 ? Lua.ORDER_CONCATENATION :
      Lua.ORDER_ATOMIC;
  return [code, order];
};
...
Lua['text_join'] = function(block) {
  ...
  if (block.itemCount_ === 0) {
    ...
    const element = Lua.valueToCode(block, 'ADD0',
        Lua.ORDER_NONE) || '\'\'';
    const code = 'tostring(' + element + ')';
    return [code, Lua.ORDER_HIGH];
  } else if (block.itemCount_ === 2) {
    const element0 = Lua.valueToCode(block, 'ADD0',
        Lua.ORDER_CONCATENATION) || '\'\'';
    const element1 = Lua.valueToCode(block, 'ADD1',
        Lua.ORDER_CONCATENATION) || '\'\'';
...
Lua['text_append'] = function(block) {
  ...
  const value = Lua.valueToCode(block, 'TEXT',
      Lua.ORDER_CONCATENATION) || '\'\'';
...
Lua['text_length'] = function(block) {
  ...
  const text = Lua.valueToCode(block, 'VALUE',
      Lua.ORDER_UNARY) || '\'\'';
...
Lua['text_indexOf'] = function(block) {
  ...
  const substring = Lua.valueToCode(block, 'FIND',
      Lua.ORDER_NONE) || '\'\'';
  const text = Lua.valueToCode(block, 'VALUE',
      Lua.ORDER_NONE) || '\'\'';
  let functionName;
...
Lua['text_charAt'] = function(block) {
  ...
  const atOrder = (where === 'FROM_END') ? Lua.ORDER_UNARY :
      Lua.ORDER_NONE;
...
Lua['text_getSubstring'] = function(block) {
  // Get substring.
  const text = Lua.valueToCode(block, 'STRING',
      Lua.ORDER_NONE) || '\'\'';

  // Get start index.
  const where1 = block.getFieldValue('WHERE1');
  const at1Order = (where1 === 'FROM_END') ? Lua.ORDER_UNARY :
      Lua.ORDER_NONE;
  ...
  const at2Order = (where2 === 'FROM_END') ? Lua.ORDER_UNARY :
      Lua.ORDER_NONE;
...
Lua['text_changeCase'] = function(block) {
  ...
  const text = Lua.valueToCode(block, 'TEXT',
      Lua.ORDER_NONE) || '\'\'';
  let functionName;
...
Lua['text_print'] = function(block) {
  // Print statement.
  const msg = Lua.valueToCode(block, 'TEXT',
      Lua.ORDER_NONE) || '\'\'';
  return 'print(' + msg + ')\n';
};
...
Lua['text_prompt_ext'] = function(block) {
  ...
  if (block.getField('TEXT')) {
    ...
    // External message.
    msg = Lua.valueToCode(block, 'TEXT',
        Lua.ORDER_NONE) || '\'\'';
  }
...
Lua['text_count'] = function(block) {
  const text = Lua.valueToCode(block, 'TEXT',
      Lua.ORDER_NONE) || '\'\'';
  const sub = Lua.valueToCode(block, 'SUB',
      Lua.ORDER_NONE) || '\'\'';
  const functionName = Lua.provideFunction_(
      'text_count',
      ['function ' + Lua.FUNCTION_NAME_PLACEHOLDER_
...
Lua['text_replace'] = function(block) {
  const text = Lua.valueToCode(block, 'TEXT',
      Lua.ORDER_NONE) || '\'\'';
  const from = Lua.valueToCode(block, 'FROM',
      Lua.ORDER_NONE) || '\'\'';
  const to = Lua.valueToCode(block, 'TO',
      Lua.ORDER_NONE) || '\'\'';
  const functionName = Lua.provideFunction_(
      'text_replace',
      ['function ' + Lua.FUNCTION_NAME_PLACEHOLDER_
