...
Blockly.Blocks['procedures_ifreturn'] = {
  ...
  domToMutation: function(xmlElement) {
    ...
    if (!this.hasReturnValue_) {
      ...
      if (this.shadowChild_) {
        this.shadowChild_.dispose();
        this.shadowChild_ = undefined;
      }
    }
  },
  /**
  ...
  onchange: function(_e) {
    ...
    if (legal) {
      ...
      if (block.type == 'procedures_defnoreturn' && this.hasReturnValue_) {
        ...
        var shadowChild = this.workspace.newBlock('logic_boolean');
        shadowChild.setFieldValue('TRUE', 'BOOL');
        shadowChild.setShadow(true);
        shadowChild.initSvg();
        shadowChild.render();

        // Connect shadow block to last input.
        var shadowChildConnection = shadowChild.outputConnection;
        var parentConnection = this.getInput('VALUE').connection;
        parentConnection.connect(shadowChildConnection);

        this.shadowChild_ = shadowChild;
      }
...
Blockly.Blocks['procedures_fable_return'] = {
  /**
   * Block for conditionally returning a value from a procedure.
   * @this {Blockly.Block}
   */
  init: function () {
    ...
    this.setNextStatement(true);
    this.setStyle('procedure_blocks');
    this.setTooltip(Blockly.Msg['FABLE_PROCEDURES_RETURN_TOOLTIP']);
    this.setHelpUrl(Blockly.Msg['PROCEDURES_IFRETURN_HELPURL']);
    this.hasReturnValue_ = true;
  },
  ensureSearchKeywords: function () {
    ...
    Blockly.Search.preprocessSearchKeywords('procedures_fable_return', keywords, toolboxKeywords);
  },
  /**
   * Create XML to represent whether this block has a return value.
   * @return {!Element} XML storage element.
   ...
   * List of block types that are functions and thus do not need warnings.
   * To add a new function type add this to your code:
   * Blockly.Blocks['procedures_fable_return'].FUNCTION_TYPES.push('custom_func');
   */
  FUNCTION_TYPES: ['procedures_defnoreturn', 'procedures_defreturn']
};
