...
suite('Events', function() {
  ...
  suite('Constructors', function() {
    ...
    test('UI event without block', function() {
      var event = new Events.UiBase(this.workspace.id);
    ...
    test('Click without block', function() {
      var event = new Events.Click(null, this.workspace.id, 'workspace');
      assertEventEquals(event, Events.CLICK, this.workspace.id, null, {
    ...
    test('Old UI event without block', function() {
      ...
      var event = new Events.Ui(null, 'foo', 'bar', 'baz');
      assertEventEquals(event, Events.UI, '', null, {
        'element': 'foo',
        'oldValue': 'bar',
        'newValue': 'baz',
        'recordUndo': false,
    ...
    suite('With simple blocks', function() {
      ...
      test('Block base', function() {
        var event = new Events.BlockBase(this.block);
      ...
      test('Block create', function() {
        var event = new Events.BlockCreate(this.block);
        sinon.assert.calledOnce(this.genUidStub);
        assertEventEquals(event, Events.BLOCK_CREATE,
            this.workspace.id, this.TEST_BLOCK_ID,
            {
              'recordUndo': true,
              'group': '',
      ...
      test('Block delete', function() {
        var event = new Events.BlockDelete(this.block);
        sinon.assert.calledOnce(this.genUidStub);
        assertEventEquals(event, Events.BLOCK_DELETE,
            this.workspace.id, this.TEST_BLOCK_ID,
            {
              'recordUndo': true,
              'group': '',
      ...
      test('Old UI event with block', function() {
        var TEST_GROUP_ID = 'testGroup';
        eventUtils.setGroup(TEST_GROUP_ID);
        var event = new Events.Ui(this.block, 'foo', 'bar', 'baz');
        sinon.assert.calledOnce(this.genUidStub);
        assertEventEquals(event, Events.UI, this.workspace.id,
            this.TEST_BLOCK_ID,
            {
              'element': 'foo',
              'oldValue': 'bar',
      ...
      test('Click with block', function() {
        var TEST_GROUP_ID = 'testGroup';
        eventUtils.setGroup(TEST_GROUP_ID);
        var event = new Events.Click(this.block, null, 'block');
        assertEventEquals(event, Events.CLICK, this.workspace.id,
            this.TEST_BLOCK_ID, {
              'targetType': 'block',
              'recordUndo': false,
              'group': TEST_GROUP_ID
      ...
      suite('Block Move', function() {
        test('by coordinate', function() {
          ...
          var event = new Events.BlockMove(this.block);
          sinon.assert.calledOnce(this.genUidStub);
          assertEventEquals(event, Events.BLOCK_MOVE, this.workspace.id,
              this.TEST_BLOCK_ID, {
    ...
    suite('With shadow blocks', function() {
      ...
      test('Block change', function() {
        var event = new Events.BlockChange(
            this.block, 'field', 'FIELD_NAME', 'old', 'new');
        sinon.assert.calledOnce(this.genUidStub);
        assertEventEquals(event, Events.BLOCK_CHANGE,
            this.workspace.id, this.TEST_BLOCK_ID,
            {
              'varId': undefined,
              'element': 'field',
    ...
    suite('With variable getter blocks', function() {
      ...
      test('Block change', function() {
        ...
        assertEventEquals(event, Events.BLOCK_CHANGE, this.workspace.id,
            this.TEST_BLOCK_ID,
            {
              'element': 'field',
              'name': 'VAR',
  ...
  suite('Serialization', function() {
    ...
    var uiEventTestCases = [
      {title: 'Bubble open', class: Events.BubbleOpen,
      ...
      {title: 'null to Block Marker move', class: Events.MarkerMove,
        ...
        getExpectedJson: (thisObj) => ({type: 'marker_move',
          ...
          newNode: new ASTNode(ASTNode.types.BLOCK,
              thisObj.block)})},
      {title: 'null to Workspace Marker move', class: Events.MarkerMove,
      ...
      {title: 'Block to Workspace Marker move',
        ...
        getArgs: (thisObj) => [null, true,
          ...
          ASTNode.createWorkspaceNode(thisObj.workspace,
              new Blockly.utils.Coordinate(0, 0))]},
      {title: 'Selected', class: Events.Selected,
      ...
      {title: 'Selected (deselect)', class: Events.Selected,
      ...
      {title: 'Theme Change', class: Events.ThemeChange,
      ...
      {title: 'Toolbox item select (deselect)',
        ...
        getExpectedJson: () => ({type: 'toolbox_item_select', oldItem: 'Math',
          newItem: null})},
      {title: 'Trashcan open', class: Events.TrashcanOpen,
        getArgs: (thisObj) => [true, thisObj.workspace.id],
        getExpectedJson: () => ({type: 'trashcan_open', isOpen: true})},
      {title: 'Viewport change', class: Events.ViewportChange,
      ...
      {title: 'Viewport change (0,0)', class: Events.ViewportChange,
    ...
    var workspaceEventTestCases = [
      {title: 'Finished Loading', class: Events.FinishedLoading,
    ...
    var workspaceCommentEventTestCases = [
      {title: 'Comment change', class: Events.CommentChange,
        ...
        getExpectedJson: (thisObj) => ({type: 'comment_change',
          ...
          newContents: 'foo'})},
      {title: 'Comment create', class: Events.CommentCreate,
      ...
      {title: 'Comment delete', class: Events.CommentDelete,
        getArgs: (thisObj) => [thisObj.comment],
        getExpectedJson: (thisObj) => ({type: 'comment_delete',
          commentId: thisObj.comment.id})},
      // TODO(#4577) Test serialization of move event coordinate properties.
      {title: 'Comment move', class: Events.CommentMove,
        getArgs: (thisObj) => [thisObj.comment],
  ...
  suite('Variable events', function() {
    ...
    suite('Constructors', function() {
      test('Var base', function() {
        var event = new Events.VarBase(this.variable);
        assertEventEquals(event, undefined, this.workspace.id, undefined, {
          'varId': 'id1',
          'recordUndo': true,
          'group': ''
      ...
      test('Var create', function() {
        var event = new Events.VarCreate(this.variable);
        assertEventEquals(event, Events.VAR_CREATE, this.workspace.id,
            undefined,
            {
              'varId': 'id1',
              'varType': 'type1',
      ...
      test('Var delete', function() {
        var event = new Events.VarDelete(this.variable);
        assertEventEquals(event, Events.VAR_DELETE, this.workspace.id,
            undefined,
            {
              'varId': 'id1',
              'varType': 'type1',
      ...
      test('Var rename', function() {
        var event = new Events.VarRename(this.variable, 'name2');
        assertEventEquals(event, Events.VAR_RENAME, this.workspace.id,
            undefined,
            {
              'varId': 'id1',
              'oldName': 'name1',
    ...
    suite('Run Forward', function() {
      test('Var create', function() {
        var json = {type: "var_create", varId: "id2", varType: "type2",
          varName: "name2"};
        var event = eventUtils.fromJson(json, this.workspace);
  ...
  suite('Filters', function() {
    function addMoveEvent(events, block, newX, newY) {
      events.push(new Events.BlockMove(block));
    ...
    test('No removed, order unchanged', function() {
      ...
      var events = [
        new Events.BlockCreate(block),
        new Events.BlockMove(block),
        new Events.BlockChange(block, 'field', 'VAR', 'id1', 'id2'),
        new Events.Click(block)
      ];
      var filteredEvents = eventUtils.filter(events, true);
      chai.assert.equal(filteredEvents.length, 4);  // no event should have been removed.
      // test that the order hasn't changed
      chai.assert.isTrue(filteredEvents[0] instanceof Events.BlockCreate);
      chai.assert.isTrue(filteredEvents[1] instanceof Events.BlockMove);
      chai.assert.isTrue(filteredEvents[2] instanceof Events.BlockChange);
      chai.assert.isTrue(filteredEvents[3] instanceof Events.Click);
    });
    ...
    test('Different blocks no removed', function() {
      ...
      var events = [
        new Events.BlockCreate(block1),
        new Events.BlockMove(block1),
        new Events.BlockCreate(block2),
        new Events.BlockMove(block2)
      ];
      var filteredEvents = eventUtils.filter(events, true);
      chai.assert.equal(filteredEvents.length, 4);  // no event should have been removed.
    });
    ...
    test('Forward', function() {
      var block = this.workspace.newBlock('field_variable_test_block', '1');
      var events = [ new Events.BlockCreate(block) ];
    ...
    test('Backward', function() {
      ...
      addMoveEvent(events, block, 2, 2);
      addMoveEvent(events, block, 3, 3);
      var filteredEvents = eventUtils.filter(events, false);
    ...
    test('Merge block change events', function() {
      var block1 = this.workspace.newBlock('field_variable_test_block', '1');
      var events = [
        new Events.BlockChange(block1, 'field', 'VAR', 'item', 'item1'),
        new Events.BlockChange(block1, 'field', 'VAR', 'item1', 'item2')
      ];
      var filteredEvents = eventUtils.filter(events, true);
    ...
    test('Merge viewport change events', function() {
      var events = [
        new Events.ViewportChange(1, 2, 3, this.workspace, 4),
        new Events.ViewportChange(5, 6, 7, this.workspace, 8)
      ];
      var filteredEvents = eventUtils.filter(events, true);
      chai.assert.equal(filteredEvents.length, 1);  // second change event merged into first
    ...
    test('Merge ui events', function() {
      ...
      var events = [
        new Events.BubbleOpen(block1, true, 'comment'),
        new Events.Click(block1),
        new Events.BubbleOpen(block2, true, 'mutator'),
        new Events.Click(block2),
        new Events.BubbleOpen(block3, true, 'warning'),
        new Events.Click(block3)
      ];
      var filteredEvents = eventUtils.filter(events, true);
      // click event merged into corresponding *Open event
      chai.assert.equal(filteredEvents.length, 3);
      chai.assert.isTrue(filteredEvents[0] instanceof Events.BubbleOpen);
      chai.assert.isTrue(filteredEvents[1] instanceof Events.BubbleOpen);
      chai.assert.isTrue(filteredEvents[2] instanceof Events.BubbleOpen);
    ...
    test('Colliding events not dropped', function() {
      ...
      var events = [
        new Events.Click(block),
        new Events.Ui(block, 'stackclick', undefined, undefined)
      ];
      var filteredEvents = eventUtils.filter(events, true);
      // click and stackclick should both exist
      chai.assert.equal(filteredEvents.length, 2);
      chai.assert.isTrue(filteredEvents[0] instanceof Events.Click);
    ...
    test('Move events different blocks not merged', function() {
      ...
      events.push(new Events.BlockDelete(block2));
      ...
      // test that the order hasn't changed
      chai.assert.isTrue(filteredEvents[0] instanceof Events.BlockMove);
      chai.assert.isTrue(filteredEvents[1] instanceof Events.BlockMove);
      chai.assert.isTrue(filteredEvents[2] instanceof Events.BlockDelete);
      chai.assert.isTrue(filteredEvents[3] instanceof Events.BlockMove);
    });
  });
  ...
  suite('Firing', function() {
    ...
    test('Block dispose triggers Delete', function() {
      try {
        ...
        assertNthCallEventArgEquals(
            this.eventsFireSpy, 0, Events.BlockDelete,
            {oldXml: expectedOldXml, group: ''},
            workspaceSvg.id, expectedId);
        assertNthCallEventArgEquals(
            changeListenerSpy, 0, Events.BlockDelete,
            {oldXml: expectedOldXml, group: ''},
            workspaceSvg.id, expectedId);
    ...
    test('New block new var xml', function() {
      ...
      assertNthCallEventArgEquals(
          this.changeListenerSpy, 2, Events.FinishedLoading,
